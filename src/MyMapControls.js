...
// This set of controls performs orbiting, panning, and mousewheel-zooming.
// Unlike OrbitControls, it allows rotating around the viewing perpendicular.
// Useful for top-down maps.
//
//    Rotate viewing plane - left mouse
//    Pan - right mouse
//    Tilt viewing plane - middle mouse
//    Zoom - mousewheel
//
// TODO: should also add touch controls back in

const _changeEvent = { type: 'change' };
const _startEvent = { type: 'start' };
const _endEvent = { type: 'end' };

class MyMapControls extends EventDispatcher {

	constructor( object, domElement ) {

		super();

		if ( domElement === undefined ) console.warn( 'MyMapControls: The second parameter "domElement" is now mandatory.' );
		if ( domElement === document ) console.error( 'MyMapControls: "document" should not be used as the target "domElement". Please use "renderer.domElement" instead.' );
		...
		// Mouse buttons
		this.mouseButtons = { LEFT: MOUSE.LEFT, RIGHT: MOUSE.RIGHT, MIDDLE: MOUSE.MIDDLE };
		...
		this.update = function () {
			...
			return function update() {
				...
				if ( scope.enableDamping === true ) {
                    ...
                    rotatePanAngleDelta *= ( 1 - scope.dampingFactor );
        ...
        let rotatePanAngle = 0.0;
        let rotatePanAngleDelta = 0.0;
		...
		const rotatePanStart = new Vector2();
		const rotatePanEnd = new Vector2();
		const rotatePanDelta = new Vector2();
        ...
        function rotatePan( angle ) {
            rotatePanAngleDelta += angle;
        }
		...
		function handleMouseDownRotatePan( event ) {

			rotatePanStart.set( event.clientX, event.clientY );
			rotateStart.set( event.clientX, event.clientY );

		}

		function handleMouseDownRotateTilt( event ) {
		...
		function handleMouseMoveRotatePan( event ) {

			const element = scope.domElement;

			rotatePanEnd.set( event.clientX, event.clientY );
			rotatePanDelta.subVectors( rotatePanEnd, rotatePanStart ).multiplyScalar( scope.rotateSpeed );

            rotatePan( 2 * Math.PI * rotatePanDelta.x / element.clientHeight ); // yes, height
		...
		function handleMouseMoveRotateTilt( event ) {
		...
		function onMouseDown( event ) {
            ...
            const altMode = event.ctrlKey || event.metaKey || event.shiftKey;
			switch ( event.button ) {
		...
		function onMouseMove( event ) {
			...
			switch ( state ) {
				...
				case STATE.RTILT:
					...
					handleMouseMoveRotateTilt( event );
